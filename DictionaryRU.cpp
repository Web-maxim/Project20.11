...
void DictionaryRU::loadFromFile(const string& filename) {
    ifstream file(filename); // Изменено: текстовый режим вместо ios::binary
    if (!file.is_open()) {
...
bool DictionaryRU::contains(const string& word) {
    // Изменено: используем прямой поиск в Trie вместо обхода автодополнений
    return trie.search(utf8_to_wstring(word));
}