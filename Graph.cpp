...
void Graph::findMinDistancesFloyd(string name_user) {
    // Изменено: убрана сырая куча (int**). Используем вектор для защиты от утечек.
    ...
    for (int k = 0; k < size_matrix; k++) {
        for (int i = 0; i < size_matrix; i++) {
            for (int j = 0; j < size_matrix; j++) {
                // Изменено: защита от переполнения «INF + X»
...
void Graph::print_other(const vector<vector<int>>& matrix_v, string name_user) {
    // Изменено: упрощён и ускорен подсчёт «других» пользователей, без вложенных полных циклов и set
